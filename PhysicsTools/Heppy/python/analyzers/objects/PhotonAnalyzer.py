...
class PhotonAnalyzer( Analyzer ):
    ...
    def makePhotons(self, event):
        ...
        for gamma in event.allphotons:
            ...
            #https://twiki.cern.ch/twiki/bin/view/CMS/CutBasedPhotonIdentificationRun2?rev=24
            if   abs(gamma.eta()) < 1.0:   gamma.EffectiveArea03 = [ 0, 0.0599, 0.1271 ]
            elif abs(gamma.eta()) < 1.479: gamma.EffectiveArea03 = [ 0, 0.0819, 0.1101 ]
            elif abs(gamma.eta()) < 2.0:   gamma.EffectiveArea03 = [ 0, 0.0696, 0.0756 ]
            elif abs(gamma.eta()) < 2.2:   gamma.EffectiveArea03 = [ 0, 0.0360, 0.1175 ]
            elif abs(gamma.eta()) < 2.3:   gamma.EffectiveArea03 = [ 0, 0.0360, 0.1498 ]
            elif abs(gamma.eta()) < 2.4:   gamma.EffectiveArea03 = [ 0, 0.0462, 0.1857 ]
            else:                          gamma.EffectiveArea03 = [ 0, 0.0656, 0.2183 ]
